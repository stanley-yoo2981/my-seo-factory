...
# 1. 프로젝트 경로 및 API 강제 연결 (PDF 원본 로직 100% 보존)
...
# 3. Apple Haptic Studio CSS (버튼-카드 통합 및 햅틱 효과)
st.markdown("""
<style>
    ...
    if st.button("✍️\n\n포스팅 생성\n\n\n(클릭하여 실행)", key="card_post"):
        ...
        with st.status("생성 중...", expanded=True):
            ...
            if rc == 0:
                ...
                if edit_line: st.success(f"완료: {edit_line.split()[-1].strip()}")

# --- 카드 3: 데이터 분석 ---
with col3:
    if st.button("📈\n\n데이터 분석\n\n\n(클릭하여 확인)", key="card_view"):
...
st.markdown("""
<div class='guide-box'>
    <h2 style='text-align: center; margin-bottom: 50px;'>📑 워드프레스 공장 </h2>
    <h3>1. 워드프레스 임시글 확인 및 진입</h3>
    <p>포스팅 생성 완료 후, 관리자 페이지의 [글] > [모든 글] 메뉴로 접속하여 작성된 제목을 클릭합니다.</p>
    <img src="step1.png" style="width:100%; border-radius:20px; border:1px solid #eee; margin:20px 0;">
    
    <h3>2. 과장님 검수 / Rank Math SEO 설정 (중요)</h3>
    ...
    <p><b>2) 포커스 키워드 및 체크리스트:</b> 랭크매스 점수가 80점 이상이 되도록 수정합니다.</p>
