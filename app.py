...
# 💥 권한 에러 박멸: 서버 전용 상대 경로 강제 사용 (사장님 맥북 경로 흔적 삭제)
...
<style>
    ...
    /* 🌊 진짜 메아리(Ripple) 애니메이션: 밖으로 강하게 퍼져나가는 파동 */
    @keyframes echoRipple {{
        0% {{ box-shadow: 0 0 0 0 rgba(0, 0, 0, 0.2); transform: scale(1); }}
        70% {{ box-shadow: 0 0 0 50px rgba(0, 0, 0, 0); transform: scale(1.03); }}
    ...
    /* Bento Card 버튼 - 묵직한 사각형 디자인 (사장님이 싫어하시는 캡슐 스타일 퇴출) */
    div.stButton > button {{
...
with st.sidebar:
    ...
    st.write("AI 지능 코어:", "🟢" if openai_ok else "🔴")
...
def run_engine(cmd, env_ext, log_placeholder, progress_bar):
    ...
    try:
        ...
        log_placeholder.error(f"엔진 가동 오류: {e}")
        return -1, []
...
def render_step(title, content, img_list=None):
    ...
    st.markdown(f"<p style='font-size: 21px;'>{content}</p>", unsafe_allow_html=True)
    if img_list:
        for img in img_list:
...
st.markdown("<p><b>(2) 포커스 키워드 설정:</b> 제목의 첫 번째 키워드를 복사해서 넣어주면 돼요.</p>", unsafe_allow_html=True)
render_step("", "", ["step2-2.png"])

st.markdown("<p><b>(3) RankMath 80점 돌파:</b> 모든 항목이 초록색 체크(v)가 되도록 살짝만 보완해 보세요. 최종 점수 80점 이상이면 상단 노출 준비 끝이에요!</p>", unsafe_allow_html=True)
