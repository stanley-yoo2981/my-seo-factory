...
8) [신규] hero 타입 전용 네이버 뉴스 실시간 크롤링 → 프롬프트 주입
...
def get_persona_prompt(content_type: str, focus_kw: str, lsi: list,
    ...
    news_data: hero 타입일 때 fetch_naver_news()에서 가져온 뉴스 목록 (선택)
...
"""
    ...
    if content_type == "hero":
        ...
        if news_data:
            ...
            news_block = f"""
[실시간 네이버 뉴스 — 도입부 후킹에 반드시 활용]
아래는 '{focus_kw}' 키워드로 방금 검색한 최신 뉴스 기사 3건입니다.
도입부(intro_paragraphs[0])는 반드시 이 뉴스 중 하나를 인용하거나 요약하여
"최근 이 뉴스 보셨어요?", "와, 진짜 남 일 같지 않더라고요..." 형태의
실화 기반 후킹(Hooking) 문장으로 시작하세요.
...
- 뉴스 데이터가 제공된 경우, 도입부 첫 문장은 해당 뉴스 내용을 자연스럽게 녹여 쓸 것
...

{
  ...
  "intro_paragraphs": [
    "string — 첫 번째 도입 문단 (페르소나 말투로 자연스럽게 시작, hero 타입은 뉴스 후킹 필수)",
...
def generate_ai_content(focus_kw: str, lsi: list, content_type: str,
    ...
    news_data: hero 타입 전용 네이버 뉴스 데이터 (선택)
...
def main():
    ...
    # ──────────────────────────────────────────────────────────
    # [신규] hero 타입 전용: 네이버 뉴스 실시간 크롤링
    # ──────────────────────────────────────────────────────────
    news_data = []
    if content_type == "hero":
        print(f"\n[1.5/6] 네이버 뉴스 크롤링 (hero 전용, keyword='{focus_kw}')")
        news_data = fetch_naver_news(focus_kw)
        if news_data:
    ...
    html, title, permalink, description = build_html(
        ...
        news_data=news_data,  # hero 타입일 때 뉴스 데이터 전달
    )
    ...
    if content_type == "hero" and news_data:
        has_news_hook = any(
            n["title"][:10] in html or n["summary"][:20] in html
            for n in news_data if n.get("title") or n.get("summary")
        )
    else:
    ...
    if content_type == "hero":
        print(f"  📰 네이버 뉴스 크롤링: {len(news_data)}건")
